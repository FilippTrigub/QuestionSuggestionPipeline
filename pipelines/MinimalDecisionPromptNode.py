    ...
    def __init__(self, llm_key):
        prompt_template = PromptTemplate(
            prompt=f"You are an AI tasked with categorizing {config.suggestion_category}-related requests. "
                   ...
                   # f"{', '.join(config.minimal_decision_categories.__dict__.keys())}. "
                   "\nCATEGORIES:\n"
                   + '\n'.join([key + ': ' + value
                                for key, value in config.minimal_decision_categories.__dict__.items()]) +
                   ...
                   "\nRESPONSE FORMAT:\n"
                   "< explanations >;\n"
                   f"{config.minimal_decision_phrase}: < choice of category >;\n"
                   f"{config.object_for_comparison}: <object for comparison or None>;"
                   f"\n\nEXAMPLES:\n"
                   f"1:\n"
                   f"Query: Give me a similar wine to one called Estampa 2011 Estate Viognier-Chardonnay (Colchagua Valley). \n"
                   f"Response: The correct choice is {list(config.minimal_decision_categories.__dict__.keys())[0]}, "
                   f"because the user seems to want a wine similar to one called Estampa 2011 Estate Viognier-Chardonnay (Colchagua Valley). \n"
                   f"{config.minimal_decision_phrase}: {list(config.minimal_decision_categories.__dict__.keys())[0]};\n"
                   f"{config.object_for_comparison}: Estampa 2011 Estate Viognier-Chardonnay (Colchagua Valley);\n"
                   f"2:\n"
                   f"Query: I want a dry italian wine."
                   f"Response: The correct choice is {list(config.minimal_decision_categories.__dict__.keys())[1]}, "
                   f"because the use is indicating a preference based on attributes. \n"
                   f"{config.minimal_decision_phrase}: {list(config.minimal_decision_categories.__dict__.keys())[1]};\n"
                   f"{config.object_for_comparison}: None;\n"
                   "\n\nINPUT QUERY:\n"
                   "{query}\n"
        )

        super().__init__(default_prompt_template=prompt_template,
                         model_name_or_path=config.minimal_decision_tool_generative_model,
                         api_key=llm_key,
                         model_kwargs=config.minimal_decision_model_settings.__dict__
                         )
